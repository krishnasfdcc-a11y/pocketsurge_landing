...
class CLIPTextModel(nn.Module):
    ...
    def sanitize(self, weights):
        new_weights = {}
        # First pass: collect scale/bias keys for dequantization
        scale_bias_keys = {}
        for key in weights:
            if key.endswith(".scales"):
                base = key[:-7]
                scale_bias_keys.setdefault(base, {})["scales"] = key
            elif key.endswith(".biases"):
                base = key[:-7]
                scale_bias_keys.setdefault(base, {})["biases"] = key

        for key, w in weights.items():
            # Skip quantization parameters (handled during dequant)
            if key.endswith(".scales") or key.endswith(".biases"):
                continue

            # Check if this is a quantized weight that needs dequantization
            base = key[:-7] if key.endswith(".weight") else None
            if base and base in scale_bias_keys:
                s_key = scale_bias_keys[base].get("scales")
                b_key = scale_bias_keys[base].get("biases")
                if s_key and b_key:
                    w = mx.dequantize(
                        w, weights[s_key], weights[b_key], group_size=64
                    )
