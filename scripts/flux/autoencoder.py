...
class AutoEncoder(nn.Module):
    ...
    def sanitize(self, weights):
        ...
        for k, w in weights.items():
            ...
            base_k = k[:-7] if k.endswith(".weight") else None
            if base_k and base_k in scale_bias_keys:
                s_key = scale_bias_keys[base_k].get("scales")
                b_key = scale_bias_keys[base_k].get("biases")
                if s_key and b_key:
                    w = mx.dequantize(
